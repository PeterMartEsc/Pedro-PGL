...
class Punto{
    ...
    static suma(p1,p2){
        let nuevoPunto = new Punto( p1.x + p2.x, p1.y + p2.y);
        return nuevoPunto;
    }

    producto(numero){
        let nuevoPunto = new Punto(this.x*numero, this.y*numero);
        return nuevoPunto;
    }
}
...
class Jugador{
    ...
    mover(desplazamiento){
        ...
        //la suma de la posición anterior con el desplazamiento nuevo
        let nuevaPosiblePosicion = Punto.suma(this.posicion,desplazamiento);
        

        //si la nueva posición se sale del tablero se reubica 
        //reflejando que un muro impide salir
        this.posicion = this.escenario.ponerPuntoDentro(nuevaPosiblePosicion);
    }

    moverPorInercia(){
        //para calcular el desplazamiento que tiene lugar por la inercia del movimiento
        //multiplicamos la dirección de la inercia: direccionInercia por la velocidad: pasoInercia
        let desplazamiento = this.direccionInercia.producto(this.pasoInercia);


        //la suma de la posición anterior con el desplazamiento por inercia nuevo
        let nuevaPosiblePosicion = Punto.suma(this.posicion,desplazamiento);
        

        let nuevaUbicacion = this.escenario.ponerPuntoDentro(nuevaPosiblePosicion);

        if(nuevaUbicacion.x != nuevaPosiblePosicion.x ){ // se ha chocado con un muro vertical Hay que "rebotar" 
            this.direccionInercia.x = -1 * this.direccionInercia.x;
        }

        if(nuevaUbicacion.y != nuevaPosiblePosicion.y ){ // se ha chocado con un muro horizontal Hay que "rebotar" 
            this.direccionInercia.y = -1 * this.direccionInercia.y;
        }        

        //Ponemos nuestro objeto en la nueva ubicación:
