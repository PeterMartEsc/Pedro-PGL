...
export type StackParamList = {
    AdminAlumnosList: undefined,
    AlumnosCreate: undefined,
}

type PropsHome = NativeStackScreenProps<StackParamList, 'AdminAlumnosList'>
const AdminAlumnosList = ({ navigation, route }: PropsHome) => {
    ...
    useEffect(() => {
        ...
        async function getAlumnos() {
            const response = await axios.get(`${ipRoute}/api/v3/alumnos`,
                {
                    headers: {
                        ...
                        <TouchableOpacity onPress={() => navigation.navigate('AlumnosCreate'/*, {idPokemon: item.id}*/)}>
                            <View /*style={styles.nameList}*/>
                                <Text>
                                    ...
                                    {/* <FlatList 
                                    data={item.matriculas}
                                    renderItem={({ item, index }) => { return (<Text>{item.nombre}</Text>)}}
                                    /> */}
                                </Text>
                            </View>
                            {/* 
...
export default AdminAlumnosList
